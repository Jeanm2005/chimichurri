...
class TestRunQuery(unittest.TestCase):
    ...
    @patch("data_fetcher._get_client")  # FIX: was @patch("data_fetcher.client")
    def test_returns_list_of_dicts(self, mock_get_client):
        ...
        mock_get_client.return_value.query.return_value = mock_job
    ...
    @patch("data_fetcher._get_client")  # FIX: was @patch("data_fetcher.client")
    def test_passes_params_to_job_config(self, mock_get_client):
        ...
        _, kwargs = mock_get_client.return_value.query.call_args
...
class TestGetGenaiAdvice(unittest.TestCase):
    ...
    @patch("data_fetcher._get_anthropic_client")  # FIX: was @patch("data_fetcher.anthropic_client")
    @patch("data_fetcher.get_user_activity")
    def test_returns_string(self, mock_activity, mock_get_anthropic):
        mock_activity.return_value = [
            {"sport": "Soccer", "duration_minutes": 60, "activity_type": "join_event"}
        ]
        mock_get_anthropic.return_value.messages.create.return_value = MagicMock(
    ...
    @patch("data_fetcher._get_anthropic_client")  # FIX: was @patch("data_fetcher.anthropic_client")
    @patch("data_fetcher.get_user_activity")
    def test_calls_anthropic_api(self, mock_activity, mock_get_anthropic):
        mock_activity.return_value = []
        mock_get_anthropic.return_value.messages.create.return_value = MagicMock(
            content=[MagicMock(text="Get out there and play!")]
        )
        df.get_genai_advice("u1")
        mock_get_anthropic.return_value.messages.create.assert_called_once()
 
    @patch("data_fetcher._get_anthropic_client")  # FIX: was @patch("data_fetcher.anthropic_client")
    @patch("data_fetcher.get_user_activity")
    def test_calls_get_user_activity(self, mock_activity, mock_get_anthropic):
        mock_activity.return_value = []
        mock_get_anthropic.return_value.messages.create.return_value = MagicMock(
    ...
    @patch("data_fetcher._get_anthropic_client")  # FIX: was @patch("data_fetcher.anthropic_client")
    @patch("data_fetcher.get_user_activity")
    def test_works_with_no_activity(self, mock_activity, mock_get_anthropic):
        mock_activity.return_value = []
        mock_get_anthropic.return_value.messages.create.return_value = MagicMock(
    ...
    @patch("data_fetcher._get_anthropic_client")  # FIX: was @patch("data_fetcher.anthropic_client")
    @patch("data_fetcher.get_user_activity")
    def test_returns_model_text(self, mock_activity, mock_get_anthropic):
        mock_activity.return_value = []
