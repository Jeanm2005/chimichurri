...
def _rand_ts(days_offset_min: int = -60, days_offset_max: int = 60) -> datetime:
    ...
    delta = timedelta(days=random.randint(days_offset_min, days_offset_max),
        hours=random.randit(0, 23),
...
def generate_users(n: int = 20) -> list[dict]:
    ...
    for _ in range(n):
        ...
        sports = [{"sport": s, "skill_level": random.choice(SKILL_LEVELS)}
        ...
        users.append({
            ...
            "home_lat":   lat,
            "home_lng":   lng,
            # home_geog is computed in BigQuery via ST_GEOGPOINT at insert time
            "sports":     sports,
        })
    return users
...
def generate_locations(n: int = 15) -> list[dict]:
    ...
    for i in range(n):
        ...
        locations.append({
            ...
            "lat":         lat,
            "lng":         lng,
            # geog computed in BigQuery
            "created_at":  _rand_ts(-365, -30).isoformat(),
            "created_by":  None,   # will be patched with a real user_id after user gen
...
def generate_events(
    ...
    for _ in range(n):
        ...
        # Status depends on time: past events lean toward 'completed'
        now = datetime.now(timezone.utc)
        if end_time < now:
...
def generate_friendships(user_ids: list[str], max_pairs: int | None = none) -> list[dict]:
    ...
    for i, uid in enumerate(shuffled):
        ...
        num_friends = random.randit(1, min(5, len(candidates)))
        for friend in random.sample(candidates, num_friends):
            ...
            status = random.choices(
                FRIENDSHIP_STATUSES, weights=[10, 80, 10]
            )[0]
...
def generate_user_activity(
    ...
    for user_id in user_ids:
        for _ in range(rows_per_user):
            ...
            lng = _rand.lng()
            ts = _rand_ts(-60, 0)
            duration = (random.randit(30, 180) if activity_type == "session_complete" else None)
            event_id = random.choice(event_ids) if activity_type in ("joing_event", "leave_event", "view_event") else None
    ...
    for user_id in user_ids:
        ...
        for event_id in sampled_events:
            rows.append({
                "user_id":      user_id,
                "event_id":     event_id,
                "score":        round(random.betavariate(5, 2), 4),  # skews high
...
def _bq_insert_users(client: bigquery.Client, users: list[dict]) -> None:
    ...
    for u in users:
        rows.append({
            ...
            "home_lng":   u["home_lng"],
            "sports":     u["sports"],
            # home_geog cannot be set via insert_rows_json (GEOGRAPHY type);
            # run an UPDATE DML after seeding if needed, or use a load job.
        })
...
def _bq_insert_locations(client: bigquery.Client, locations: list[dict]) -> None:
    ...
    for loc in locations:
        rows.append({
            ...
            # geog: same note as home_geog above
        })
    ...
    for e in events:
        ...
        rows.append({
            ...
            "location": {
                ...
                "lat":         loc["lat"],
                "lng":         loc["lng"],
                # geog: must be set via DML separately
            },
